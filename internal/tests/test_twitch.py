...
from internal.views.twitch import TwitchView
from reseller.settings import EL_PAGINATION_PER_PAGE

class TwitchViewTest(TestCase):
    def setUp(self):
        self.order = None
        ...
        self.group.permissions.add(
            Permission.objects.get(
                ...
                codename=Perms.CAN_VIEW_TWITCH_ORDERS
            )
        )
        self.group.save()
        ...
        self.request = RequestFactory().get(reverse(f"internal:{Platform.TWITCH}"))
    ...
    def test_ok(self):
        self.request.user = self.user
        response = TwitchView.as_view()(self.request)
