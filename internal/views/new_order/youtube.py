...
from pyreydenx.model.new_order_parameters import YouTubeOrder
...
_check_perms = partial(
    check_perms,
    perms=(Perms.CAN_CREATE_YOUTUBE_ORDERS,),
)


@method_decorator(_check_perms, name="dispatch")
class NewYouTube(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request: HttpRequest, *args: Any, **kwargs: Any) -> TemplateResponse:
        ...
        context = {
            "title": _("New YouTube Order"),
            "platform": Platform.YOUTUBE,
            "categories": "",
            "prices": "",
            "breadcrumbs": [
                {
                    "name": _("YouTube Orders"),
                    "url": reverse_lazy(f"internal:{Platform.YOUTUBE}"),
                },
                {"name": _("New YouTube Order"), "url": None},
            ],
        }
        ...
        prices = []
        tariffs = (
            Tariff.objects.filter(is_active=True, platform=Platform.YOUTUBE)
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> JsonResponse:
        try:
            parameters = YouTubeOrder(**json.loads(request.body.decode()))
            client = Client()
